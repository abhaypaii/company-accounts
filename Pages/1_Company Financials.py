...
if not st.session_state.disable:
    ...
    with tab1:
        try:
            ...
            cols = st.columns(5)
            with cols[0]:
                st.metric("EBITDA", value = "$"+millify(financials.loc["Normalized EBITDA", '2023'], precision = 2), 
                        delta = millify(financials.loc["Normalized EBITDA", '2023'] - financials.loc["Normalized EBITDA", '2022'], precision = 2))
            with cols[1]:
                st.metric("Net Income", value = "$"+millify(financials.loc["Net Income", '2023'], precision = 2), 
                        delta = millify(financials.loc["Net Income", '2023'] - financials.loc["Net Income", '2022'], precision = 2))
            with cols[2]:
                st.metric("Total Revenue", value = "$"+millify(financials.loc["Total Revenue", '2023'], precision = 2), 
                        delta = millify(financials.loc["Total Revenue", '2023'] - financials.loc["Total Revenue", '2022'], precision = 2))
            with cols[3]:
                st.metric("Earnings Per Share", value = "$"+millify(financials.loc["Diluted EPS", '2023'], precision = 2), 
                        delta = millify(financials.loc["Diluted EPS", '2023'] - financials.loc["Diluted EPS", '2022'], precision = 2))
            with cols[4]:
                roe1 = financials.loc["Net Income", '2023']/balancesheet.loc["Stockholders Equity", '2023']
                roe0 = financials.loc["Net Income", '2022']/balancesheet.loc["Stockholders Equity", '2022']
    ...
    with tab2:
        try:
            cols = st.columns(5)
            with cols[0]:
                fcf1 = cashflow.loc['Free Cash Flow', '2023']
                fcf0 = cashflow.loc['Free Cash Flow', '2022']
                st.metric("Free Cash Flow", value = "$"+millify(fcf1, precision=2), delta = millify(fcf1-fcf0, precision=2) )
            with cols[1]:
                ocf1 = cashflow.loc['Operating Cash Flow', '2023']
                ocf0 = cashflow.loc['Operating Cash Flow', '2022']
                st.metric("Operating Cash Flow", value = "$"+millify(ocf1, precision=2), delta = millify(ocf1-ocf0, precision=2) )
            with cols[2]:
                icf1 = cashflow.loc['Investing Cash Flow', '2023']
                icf0 = cashflow.loc['Investing Cash Flow', '2022']
                st.metric("Investing Cash Flow", value = "$"+millify(icf1, precision=2), delta = millify(icf1-icf0, precision=2) )
            with cols[3]:
                fincf1 = cashflow.loc['Financing Cash Flow', '2023']
                fincf0 = cashflow.loc['Financing Cash Flow', '2022']
    ...
    with tab3:
            try:
                cols = st.columns(6)
                with cols[0]:
                    se1 = balancesheet.loc["Stockholders Equity", "2023"]
                    se0 = balancesheet.loc["Stockholders Equity", "2022"]
                    st.metric("Stockholders Equity", value="$"+millify(balancesheet.loc["Stockholders Equity", "2023"], precision = 2), 
                            delta = millify(balancesheet.loc["Stockholders Equity", "2023"] - balancesheet.loc["Stockholders Equity", "2022"]))
                with cols[1]:
                    st.metric("Retained Earnings", value="$"+millify(balancesheet.loc["Retained Earnings", "2023"], precision = 2), 
                            delta = millify(balancesheet.loc["Retained Earnings", "2023"] - balancesheet.loc["Retained Earnings", "2022"]))
                with cols[2]:
                    ta1 = balancesheet.loc["Total Assets", "2023"]
                    ta0 = balancesheet.loc["Total Assets", "2022"]
                    tl1 = ta1-se1
                    tl0 = ta0-se0
                    st.metric("Total Assets", value="$"+millify(balancesheet.loc["Total Assets", "2023"], precision = 2), 
                            delta = millify(balancesheet.loc["Total Assets", "2023"] - balancesheet.loc["Total Assets", "2022"]))
                with cols[3]:
                ...
                with cols[5]:
                    ca1, cl1 = balancesheet.loc['Current Assets', '2023'], balancesheet.loc['Current Liabilities', '2023']
                    ca0, cl0 = balancesheet.loc['Current Assets', '2022'], balancesheet.loc['Current Liabilities', '2022']
